...
namespace gpu {
	...
	struct EBO {
		...
		int update_vertex_size() {
			vertex_size = *std::max_element(data.begin(), data.end());
			return vertex_size;
		}

		friend std::ostream& operator<<(std::ostream& os, const EBO& ebo ) {
			for (int i : ebo.data) os << i << ' ';
			return os;
		}
